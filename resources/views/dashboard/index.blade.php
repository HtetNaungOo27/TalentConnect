        ...
        <main class="lg:w-2/3 space-y-8">
            ...
            <div x-data="{ open: false }" class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden transition-all hover:shadow-md">
                ...
                <div class="p-6 flex items-center justify-between bg-white">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        {{-- Only allow editing if you want --}}
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="bg-white p-4 rounded-xl border border-gray-200 flex flex-wrap md:flex-nowrap justify-between items-center gap-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <a href="{{ route('employers.show',$applicant->user) }}" class="font-bold text-gray-900">{{ $applicant->full_name }}</a>
            ...
            <div class="grid gap-4">
                <!-- @forelse($appliedJobs as $job)
                        @php
                            $application = $job->applicants->first();
                        @endphp
                ...
                <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">
                            Applied on {{ \Carbon\Carbon::parse($job->pivot->created_at)->format('M d, Y') }}
                        </p>
                    </div>

                    <span @class([ 'px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider' , 'bg-yellow-100 text-yellow-700'=> $job->pivot->status === 'pending',
                        'bg-green-100 text-green-700' => $job->pivot->status === 'accepted',
                        'bg-red-100 text-red-700' => $job->pivot->status === 'rejected',
                        ])>
                        {{ $job->pivot->status }}
                    </span>
                </div>
